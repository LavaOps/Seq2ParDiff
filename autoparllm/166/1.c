...
int dummyMethod3();
int dummyMethod4();
clock_t clock() __attribute__((nothrow));

void readfile(FILE *infile, double *val, int *col_idx, int *row_ptr,
              int *dangling_idx, int nodes, int edges) {
    int temp1;
    int temp2;
    int i;
    int j;
    ...
    fclose(infile);
    // Collecting total number of FromNode links for each website, and total
...
double dangling_PageRank(double *x_old, int *dangling_idx) {
    /*Function for calculating the sum of the pagerank-score of
     * dangling-webpages*/
...
void spmm(int nodes, double *val, int *col_idx, int *row_ptr, double *x_old,
          double *spmm_result, double d) {
...
void PageRank_iterations(double *val, int *col_idx, int *row_ptr,
                         int *dangling_idx, int nodes, double d, double W_sum,
...
int main(int narg, char **argv) {
    ...
    int n;
    // top n webpages
    /*Reading the input file and extracting the total number webpages and
     * links*/
    FILE *infile;
    ...
    /*In order to use the CRS arrays in other functions, we need to declare them
      in the main-function. Since we yet don't know what size they will have to
      be, we temporary allocate enough memory for their maximum possible size,
      and then reallocate the proper amount of memory later.*/
...
#pragma omp parallel for
    ...
    dummyMethod4();
    PageRank_iterations(val, col_idx, row_ptr, dangling_idx, nodes, d, W_sum,
                        x_old, x_new, spmm_result, one_over_N);
    dummyMethod3();
    for (int i = 0; i <= nodes - 1; i += 1) {
        printf("%lf | %lf\n", x_old[i], x_new[i]);
    }
...
spmm_result(x_0) = d*[0.0417, 0.1667, 0.0625, 0.1250, 0.1458, 0.1458, 0.1042,
0.2083] x_1 = [0.0542, 0.1604, 0.0719, 0.1250, 0.1427, 0.1427, 0.1073, 0.1958]
