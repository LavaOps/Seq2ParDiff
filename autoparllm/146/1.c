...
/*
        ...
        for (col = 0; col < (n - 1); col++)			//col is col
        {
                ...
                for (threadG = 0; threadG < rowsThr; threadG++)
                {
                        for (threadN = 0; threadN < numThr; threadN++)
                        {
                                ...
                                //printf("mainB: row: %d, col: %d, rowsThr:
   %d\n", row, col, rowsThr);
                                //struct reduceRowArgs *args =
   malloc(sizeof(*args)); rowptr = (void *)row;
                                //args->arg1 = row;
                                //args->arg2 = col;
                                pthread_create(&p_threads[threadN], &attr,
   reduceRow, rowptr);
                                //reduceRow(row, col);
                        }
                }
   ...
   col, rowsThr);
                        ...
                        //args->arg1 = row;
                        //args->arg2 = col;
                        pthread_create(&p_threads[threadN], &attr, reduceRow,
   rowptr);
                        //reduceRow(row, col);
                }
                for (i = 0; i < numThr; i++)
