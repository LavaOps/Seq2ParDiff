 ...
 * where
 *   maxiter denotes the maximum number of iterations at each point -- by
 * default 1000 x0, y0, and size specify the range to examine (a square centered
 * at (x0 + iy0) of size 2*size by 2*size -- by default, a square of size 4 by 4
 * centered at the origin) windowsize denotes the size of the image (diplay
 * window) to compute
 *
...
/* (previously processed: ignoring self-referential macro declaration) macro
 * name = N */
#define NPIXELS 800 /* size of display window in pixels */
int row;
int col;
// variables used to traverse the problem space
...
void mandelbrot(int height,int width,double real_min,double imag_min,double scale_real,double scale_imag,int maxiter,
#if _DISPLAY_
#else
    int **output)
#endif
...
#pragma omp parallel for private(col)                                          \
    firstprivate(width, real_min, imag_min, scale_real, scale_imag)
        // #pragma rose_outline
...
int main(int argc,char *argv[])
{
    ...
    for (int i = 1; i <= argc - 1; i += 1) {
        if (strcmp(argv[i], "-i") == 0) {
            ...
            if ((fp = fopen("mandel.out", "wb")) == ((void *)0)) {
            ...
            fprintf(
                stderr,
                "Usage: %s [-o -i maxiter -w windowsize -c x0 y0 -s size]\n",
                argv[0]);
            fprintf(stderr, "       -o to write computed image to disk "
                            "(default no file generated)\n");
#endif
            fprintf(stderr, "       -i to specify maximum number of iterations "
                            "at each point (default 1000)\n");
#if _DISPLAY_
#else
            fprintf(stderr, "       -w to specify the size of the image to "
                            "compute (default 800x800 elements)\n");
#endif
            fprintf(stderr, "       -c to specify the center x0+iy0 of the "
                            "square to compute (default origin)\n");
            fprintf(stderr, "       -s to specify the size of the square to "
                            "compute (default 2, i.e. size 4 by 4)\n");
            return 1;
        }
    }
    real_min = x0 - size;
    ...
    fprintf(stdout, "center = (%g, %g), size = %g\n", (real_max + real_min) / 2,
            (imag_max + imag_min) / 2, (real_max - real_min) / 2);
...
#if _DISPLAY_
#else
    mandelbrot(height, width, real_min, imag_min, scale_real, scale_imag,
               maxiter, output);
#endif
    /* End timing  */
    stamp = getusec_() - stamp;
    stamp = stamp / 1e6;
    printf("%s: %0.6fs\n", "Total execution time", stamp);
    ;
