...
void volume_append(struct volume* v, struct phaseball* o) {
    if( v->last == v->size ) {
        (v->size) += 100;
        v->objects = realloc(v->objects, sizeof(struct
phaseball*)*(v->size)+100);
    }
    (v->objects)[(v->last)] = o;
    (v->last) += 1;
    return;
...
*/
// Place phaseballs uniformly in a box, with mass equal to the manhattan
// distanced

int place_uniformly(int sx, int ex, int sy, int ey, int sz, int ez,
                    struct volume *v) {
    int counter = 0;
    ...
    for (int i = sx; i <= ex; i += 1) {
        for (int j = sy; j <= ey; j += 1) {
            for (int k = sz; k <= ez; k += 1) {
                ...
                v->z[counter] = k;
                // v->mass[i] = 1; //
                v->mass[counter] = (fabs(v->x[counter]) + fabs(v->y[counter]) +
                                    fabs(v->z[counter]));
...
int main(int argc, char **argv) {
    ...
    long msec = (end_time.tv_sec - start_time.tv_sec) * 1000 +
