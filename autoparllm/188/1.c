 ...
 *  - So we can not parallelise it. Middle loop has no dependency, so chose
 middle one to parallelise.
 ...
 * Steps to compile and execute
    ...
    2) run : gcc -o GaussianEliminationUsingOpenMP
 GaussianEliminationUsingOpenMP.c -fopenmp 3) run : qlogin -q interactive.q 4)
 run : cd hw2 5) run : ./GaussianEliminationUsingOpenMP 10 4 In step 5 :
 [argument1 (10) is MATRIX_SIZE, this is mandatory to pass. and maximum value it
 can take is 20000] [argument2 (4) is seed value, this is an optional field]
 *
 *
 */
#include "omp.h"
...
void printInputValues() {
    ...
    if (MATRIX_SIZE < 10) {
        ...
        for (row = 0; row <= MATRIX_SIZE - 1; row += 1) {
            for (col = 0; col <= MATRIX_SIZE - 1; col += 1) {
                printf("%5.2f%s", COEFFICIENT[row][col],
                       (col < MATRIX_SIZE - 1 ? ", " : ";\n\t"));
            }
        }
        dummyMethod4();
...
/* Gaussian elimination using openMp
 * Outer loop has data dependency so pragma omp parallelism has been used for
 * 1st inner loop Later on, back substitution has been used to calculate Result
 */
...
void gaussianEliminationUsingOpenMp() {
    ...
    for (iterator = 0; iterator <= MATRIX_SIZE - 1 - 1; iterator += 1) {
        ...
        for (row = iterator + 1; row <= MATRIX_SIZE - 1; row += 1) {
            ...
            multiplier =
                COEFFICIENT[row][iterator] / COEFFICIENT[iterator][iterator];
...
#pragma omp parallel for private(column)
            ...
            for (column = iterator; column <= MATRIX_SIZE - 1; column += 1) {
                ...
                COEFFICIENT[row][column] =
                    COEFFICIENT[row][column] -
                    COEFFICIENT[iterator][column] * multiplier;
            }
    ...
    for (row = MATRIX_SIZE - 1; row >= 0; row += -1) {
        ...
        for (column = MATRIX_SIZE - 1; column >= row + 1; column += -1) {
            RESULT[row] =
                RESULT[row] - COEFFICIENT[row][column] * RESULT[column];
        }
