    ...
    if (argc >= 2) {
        ...
        if (argc == 4) {
            ...
            if (pthread_count < 1 || pthread_count > N) {
                printf("Number of Threads is out of Range [It should be less "
                       "than or equal to %i] \n",
                       N);
                exit(0);
            }
        ...
        printf(
            "Usage: %s <matrix_dimension> [random seed] [Number of PThreads]\n",
            argv[0]);
        exit(0);
    }
    /* Print parameters */
...
int main(int argc, char **argv) {
    ...
    usecstart =
        ((unsigned long long)etstart.tv_sec) * 1000000 + etstart.tv_usec;
...
#pragma omp parallel for private(col) firstprivate(N)
    // #pragma rose_outline
